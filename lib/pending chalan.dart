...
// Replace with your PaidChallanScreen import if needed
// import 'package:automatic_challan_generator/paid%20chalan.dart';

class PendingChallanScreen extends StatefulWidget {
  @override
  _PendingChallanScreenState createState() => _PendingChallanScreenState();
}

class _PendingChallanScreenState extends State<PendingChallanScreen> {
  ...
  Future<void> loadAllExistingChallans() async {
    ...
    try {
      ...
      // Remove orderBy temporarily to test if ordering is causing problem
                  ...
                  Row(
                    children: [
                      TextButton.icon(
                        ...
                        onPressed: () async {
                          try {
                            await FirebaseFirestore.instance.collection('uploaded_images').doc(docId).update({
                              'status': 'Paid',
                            ...
                            setState(() {
                              challans.removeWhere((challan) => challan['docId'] == docId);
                            });

                            Navigator.pop(context);
                            // Navigate to PaidChallanScreen or just pop dialog
                            // Navigator.push(context, MaterialPageRoute(builder: (_) => PaidChallanScreen()));
                          } catch (e) {
                      ...
                      TextButton.icon(
                        ...
                        label: const Text("Unpaid", style: TextStyle(color: Colors.orange)),
                        onPressed: () async {
                          try {
                            ...
                            print("Error updating challan status to pending: $e");
                          }
                        },
                      ),
                      const Spacer(),
